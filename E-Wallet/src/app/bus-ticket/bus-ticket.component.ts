  ...
  onsubmit() {
    ...
    for (let user of this.user) {
      if (this.password != user.Password) {
        ...
        this.route.navigateByUrl('/bus-ticket')
        return
      }
      else {
        if (this.from == "bank") {
          for (let bank of this.bank) {
            ...
            var newhis: history = {
              "Name": "GSRTC",
              "Description": "Bus Ticket Successfully Booked For "+this.source+" To "+this.destination+" for "+this.count+" Person on "+this.date,
              "Credited": 0,
              "Debited": this.total,
            ...
            this.toastr.success("Bus-Ticket Successfully Booked.")
          }
        }
        else {
