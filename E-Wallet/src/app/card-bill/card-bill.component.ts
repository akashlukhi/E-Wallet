...
export class CardBillComponent implements OnInit {
  ...
  onsubmit() {
    this.card = this.form.get('card').value
    ...
    for (let user of this.user) {
      if (this.password != user.Password) {
        ...
        this.route.navigateByUrl('/card-bill')
        return
      }
      else {
        if (this.from == "bank") {
          for (let bank of this.bank) {
            ...
            var newhis: history = {
              "Name": "Card Bill",
              "Description": "Bill Paid to "+this.type +" Card Id: " + (this.card),
              "Credited": 0,
              "Debited": this.amount,
            ...
            this.toastr.success('Successfully Paid.', 'Card Bill')
          }
        }
        else {
