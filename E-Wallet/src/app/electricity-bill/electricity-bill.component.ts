...
export class ElectricityBillComponent implements OnInit {
  ...
  onsubmit() {
    this.consumer = this.form.get('consumer').value
    ...
    for (let user of this.user) {
      if (this.password != user.Password) {
        ...
        this.route.navigateByUrl('/electricity-bill')
        return
      }
      else {
        if (this.from == "bank") {
          for (let bank of this.bank) {
            ...
            var newhis: history = {
              "Name": "DGVCL",
              "Description": "Bill Paid to Consumer Id: " + (this.consumer),
              "Credited": 0,
              "Debited": this.amount,
            ...
            this.toastr.success('Successfully Paid.', 'Electricity Bill')
          }
        }
        else {
