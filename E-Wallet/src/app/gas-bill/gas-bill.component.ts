...
export class GasBillComponent implements OnInit {
  ...
  onsubmit() {
    this.customer = this.form.get('customer').value
    ...
    for (let user of this.user) {
      if (this.password != user.Password) {
        ...
        this.route.navigateByUrl('/gas-bill')
        return
      }
      else {
        if (this.from == "bank") {
          for (let bank of this.bank) {
            ...
            var newhis: history = {
              "Name": "Gas Supply",
              "Description": "Bill Paid to Customer Id: " + (this.customer),
              "Credited": 0,
              "Debited": this.amount,
            ...
            this.toastr.success('Successfully Paid.', 'Gas Bill');
          }
        }
        else {
