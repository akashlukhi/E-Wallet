...
export class MobileRechargeComponent implements OnInit {
    ...
    onsubmit() {
      ...
      this.mode = this.form.get('mode').value
      
      //Deduct money
      for (let user of this.user) {
        if (this.password != user.Password) {
          this.toastr.error('Wrong Password.')
          this.route.navigateByUrl('/mobile-recharge')
          return
        }
        else {
          if (this.from == "bank") {
            for (let bank of this.bank) {
              ...
              var newhis: history = {
                "Name": "Mobile",
                "Description": "Recharge "+this.mode +" Mobile no: " + (this.phone),
                "Credited": 0,
                "Debited": this.amount,
              ...
              this.toastr.success('Mobile Recharge Successfull.');
            }
          }
          else {
